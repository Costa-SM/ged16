...
subtitle: "AULA04: Prática (1o. semestre/2023)"
...
O primeiro dos modelos tradado é o de variáveis explicativas `length`, `diameter`e `shell_w`. 
Como a variável `age` já foi atribuída, podemos começar a matriz de gráficos de dispersão do
...
em valores menores de `age`.

Além dessa concentração de pontos, não parecem existir outras tendências na
distribuição das variâncias para ambos os modelos. Por fim, ressalta-se a 
presença de alguns *outliers* nos extremos à direita de ambos os gráficos,
porém, novamente, não parecem existir desvios sistemáticos da resposta linear.

---

Pode-se construir também gerar os gráficos de resíduos *versus* as diferentes
...
# Grafico: resid x POP*RPC
ggplot(rlm_partone_data, aes(x = length*diameter*shell_w, y = resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  ggtitle("Gráfico de resíduos vs. (length x diameter x shell_w): modelo age ~ length + diameter + shell_w") +
  labs(y = "resíduos", x = "length x diameter x shell_w")
```

Nota-se, pela análise dos gráficos gerados, em especial o referente à variável
`length` e `diameter`, que parece existir uma relação não linear entre a variável
explicativa e a resposta. Isso não parece ser o caso para a variável `shell_w`, e 
para o produto das três explicativas.
...
# Grafico: resid x height*diameter*shell_w
ggplot(rlm_parttwo_data, aes(x = height*diameter*shell_w, y = resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  ggtitle("Gráfico de resíduos vs. (height x diameter x shell_w): modelo age ~ height + diameter + shell_w") +
  labs(y = "resíduos", x = "height x diameter x shell_w")
```

Embora menos evidente que o modelo I, nos gráficos referentes às variáveis `height` 
e `diameter`, também parece existir uma relação não linear entre a variável
explicativa e a resposta. Para a primeira, evidencia-se a existência de dois
*outliers* bastante discrepantes da massa principal. Por fim, novamente, para as
variáveis `shell_w`, e o produto das três explicativas, a tendência parece ser linear.
...
apresentadas anteriormente, e pelo desvio da normalidade nos quantis finais dos resíduos.
...
Analisando-se as métricas, parece o modelo que melhor descreve o comportamento de `length` ocorre quando limitamos abalones em sexo não identificados, pois o modelo possui o menor MSE e o maior $R_{adj}^2$. Por sua vez, o modelo masculino possui o menor $R_{adj}^2$ enquanto o modelo feminino possui o maior MSE.

### Diagnósticos

Para os diagnósticos, nos limitaremos a apenas abalones masculinos e femininos
...
```
Os resíduos não aparentam clara falta de linearidade ou hetoroscedasticidade.
```{r}
#| layout-ncol: 2
...
ggplot(rlm_length_m_data, aes(x = shucked_w, y = resid)) +
  ...
  ggtitle("Gráfico de resíduos vs. shell_w: modelo length ~ age + diameter + shell_w + shucked_w para sex = M") +
  labs(y = "resíduos", x = "variável explicativa (shucked_w)")
# Grafico: resid x age*diameter*shell_w*shucked_w
ggplot(rlm_length_m_data, aes(x = age*diameter*shell_w*shucked_w, y = resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  ggtitle("Gráfico de resíduos vs. (length x diameter x shell_w): modelo age ~ length + diameter + shell_w para sex = M") +
  labs(y = "resíduos", x = "age x diameter x shell_w x shucked_w")
```
Analisando os desvios por variáveis explicativas, nota-se que age parece possuir maior variância para valores pequenos, o que tembém ocorre no produto das variáveis.
...
Observa-se desvios de normalidade mais fortes no primeiro quartil, em relação ao último.
#### Feminino
...
```
Os resíduos parecem ser relaticamente bem distribuídos, sem nenhuma tendência clara de heteroscedasticidade.
```{r}
#| layout-ncol: 2
...
ggplot(rlm_length_f_data, aes(x = shucked_w, y = resid)) +
  ...
  labs(y = "resíduos", x = "variável explicativa (shucked_w)")
# Grafico: resid x age*diameter*shell_w*shucked_w
ggplot(rlm_length_f_data, aes(x = age*diameter*shell_w*shucked_w, y = resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  ggtitle("Gráfico de resíduos vs. (length x diameter x shell_w): modelo age ~ length + diameter + shell_w para sex = F") +
  labs(y = "resíduos", x = "age x diameter x shell_w x shucked_w")
```
Analisando os desvios por variáveis explicativas, nota-se que age parece possuir maior variância para valores pequenos, o que tembém ocorre no produto das variáveis.
