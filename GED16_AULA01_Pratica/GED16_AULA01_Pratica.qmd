...
Identifando a estrutura dos dados:
```{r}
str(abalone)
```
Percebemos que as variáveis já possuem nomes explicativos. Porém, percebe-se que `sex` está com tipo `char` quando deveria ser `Factor`. Vamos também adicionar idade e retornar os valores contínuos para suas escalas originais (multiplicar por 200).
...
```
Obtemos duas observações que não fazem sentido. Retirando-as:
```{r}
