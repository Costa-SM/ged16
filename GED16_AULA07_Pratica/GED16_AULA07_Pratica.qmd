...
```{r}
# modelo polinomial de 2a. ordem completo para as variáveis quantitativas
m2_quant <- lm(age ~ I(height_c^2) + I(diameter_c^2) + I(shell_w_c^2) + height_c*shell_w_c+ diameter_c*height_c + diameter_c* shell_w_c  , data = abalone)

# resumo do modelo 
summary(m2_quant)
```

Para realizar a inclusão das variáveis categóricas, usamos variáveis _dummy_. Então, consideramos o modelo abaixo, em que as variáveis
$X_M$, $X_F$ e $X_I$ representam os diferentes níveis de `sex`.
...
E[age] = &\beta_0+\beta_1\cdot height +\beta_{11}\cdot height^2 + \beta_2\cdot diameter +\beta_{22}\cdot diameter^2\\
      +  &\beta_3\cdot shell\_w +\beta_{33}\cdot shell\_w^2 + \beta_{12}\cdot height\cdot diameter\\
      +  &\beta_{13}\cdot height\cdot shell\_w + \beta_{23}\cdot diameter\cdot shell\_w \\
      +  &\beta_M\cdot X_M+ \beta_F\cdot X_F+ \beta_I\cdot X_I
\end{align}
$$

Assim, alteramos o conjunto de dados, para realizar a inclusão das variáveis _dummy_ para cada nível de `sex`.
```{r}
# modelo polinomial de 2a. ordem completo para as variáveis quantitativas
m2_qual <- lm(age ~ I(height_c^2) + I(diameter_c^2) + I(shell_w_c^2) + height_c*shell_w_c+ diameter_c*height_c + diameter_c* shell_w_c + sex, data = abalone)
