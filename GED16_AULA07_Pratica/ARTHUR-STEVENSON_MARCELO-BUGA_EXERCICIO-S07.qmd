...
```{r}
anova(m2_qual_red_nd)
```

```{r}
#| layout-ncol: 2
# Constrói tabela com dados do modelo `m_red`
m2_qual_rednd_data <- abalone %>%
                       # inclui coluna com valores ajustados
                       mutate(fitted = m2_qual_red_nd$fit) %>%
                       mutate(resid = m2_qual_red_nd$res)

# Gera gráficos dos resíduos:
ggplot(m2_qual_rednd_data, aes(x = fitted, y = resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  ggtitle("Gráfico de Resíduos: Modelo 2 Final") +
  labs(y = "Resíduos", x = "Resposta Ajustada (age)")
# Gráfico de quantis
ggplot(m2_qual_rednd_data, aes(sample = resid)) +
  stat_qq() + stat_qq_line() +
  ggtitle("gráfico de quantis normais") +
  labs(y = "quantis amostrais", x = "quantis teóricos (dist. normal)") 
```

Nota-se, por fim, que todos os coeficientes do modelo ajudam na redução de variabilidade da resposta.
